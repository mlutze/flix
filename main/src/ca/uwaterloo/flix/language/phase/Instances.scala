  ...
  private def visitInstances(root: TypedAst.Root, oldRoot: TypedAst.Root, changeSet: ChangeSet)(implicit flix: Flix): List[InstanceError] = {
    ...
    def checkSimple(inst: TypedAst.Instance): List[InstanceError] = inst match {
      case TypedAst.Instance(_, _, _, sym, tpes, _, _, _, _) => tpes.last match { // MATT only the last one needs to be simple
        case _: Type.Cst => Nil
        case _: Type.Var => List(InstanceError.ComplexInstanceType(tpes.last, sym, sym.loc))
        case _: Type.Apply =>
          val (_, errs0) = tpes.last.typeArguments.foldLeft((List.empty[Type.Var], List.empty[InstanceError])) {
    ...
    def checkSuperInstances(inst: TypedAst.Instance): List[InstanceError] = inst match {
      case TypedAst.Instance(_, _, _, sym, tpes, tconstrs, _, _, _) =>
        val superClasses = root.classEnv(sym.clazz).tconstrs
        superClasses flatMap {
          superClass =>
            // Find the instance of the superclass matching the type of this instance.
            findInstanceForType(tpes.last, superClass.head.sym) match { // MATT need new checks for parametric TCs
              case Some((superInst, subst)) =>
                ...
                superInst.tconstrs flatMap {
                  tconstr =>
                    ClassEnvironment.entail(tconstrs.map(subst.apply), subst(tconstr), root.classEnv, RigidityEnv.empty) match { // MATT renv?
                      case Validation.Failure(errors) => errors.map {
                        case UnificationError.NoMatchingInstance(missingTconstr) => InstanceError.MissingConstraint(missingTconstr, superClass.head.sym, sym.loc)
              ...
              case None =>
                ...
                List(InstanceError.MissingSuperClassInstance(tpes.last, sym, superClass.head.sym, sym.loc))
            }
        }
    }
