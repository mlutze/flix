...
object Regions {
  ...
  private def visitDef(def0: Def)(implicit root: Root, flix: Flix): List[TypeError.RegionVarEscapes] =
    visitExp(def0.exp)(Nil, root, flix)

  private def visitExp(exp0: Expr)(implicit scope: List[Type.Var], root: Root,  flix: Flix): List[TypeError.RegionVarEscapes] = exp0 match {
  ...
  def visitJvmMethod(method: JvmMethod)(implicit scope: List[Type.Var], root: Root, flix: Flix): List[TypeError.RegionVarEscapes] = method match {
  ...
  /**
    ...
    */
  private def checkType(tpe: Type, loc: SourceLocation)(implicit scope: List[Type.Var], root: Root, flix: Flix): List[TypeError.RegionVarEscapes] = {
